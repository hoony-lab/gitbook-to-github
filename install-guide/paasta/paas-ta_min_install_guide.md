# PAAS-TA\_MIN\_INSTALL\_GUIDE

### Table of Contents

1. [개요](paas-ta_min_install_guide.md#1)  

   　● [목적](paas-ta_min_install_guide.md#1.1)  

   　● [범위](paas-ta_min_install_guide.md#1.2)  

   　● [참고 자료](paas-ta_min_install_guide.md#1.3)  

2. [PaaS-TA 5.5.1](paas-ta_min_install_guide.md#2)  
3. [PaaS-TA 5.5.1 설치](paas-ta_min_install_guide.md#3)  

   　3.1. [Prerequisite](paas-ta_min_install_guide.md#3.1)  

   　3.2. [설치 파일 다운로드](paas-ta_min_install_guide.md#3.2)  

   　3.3. [Stemcell 업로드](paas-ta_min_install_guide.md#3.3)  

   　3.4. [Runtime Config 설정](paas-ta_min_install_guide.md#3.4)  

   　3.5. [Cloud Config 설정](paas-ta_min_install_guide.md#3.5)  

   　　●  [AZs](paas-ta_min_install_guide.md#3.5.1)  

   　　●  [VM types](paas-ta_min_install_guide.md#3.5.2)  

   　　●  [Compilation](paas-ta_min_install_guide.md#3.5.3)  

   　　●  [Disk Size](paas-ta_min_install_guide.md#3.5.4)  

   　　●  [Networks](paas-ta_min_install_guide.md#3.5.5)  

   　3.6. [PaaS-TA 설치 파일](paas-ta_min_install_guide.md#3.6)  

   　　3.6.1. [PaaS-TA 설치 Variable 파일](paas-ta_min_install_guide.md#3.6.1)    

   　　　●  [common\_vars.yml](paas-ta_min_install_guide.md#3.6.1.1)  

   　　　●  [min-vars.yml](paas-ta_min_install_guide.md#3.6.1.2)  

   　　　●  [PaaS-TA 그외 Variable List](paas-ta_min_install_guide.md#3.6.1.3)  

   　　3.6.2. [PaaS-TA Operation 파일](paas-ta_min_install_guide.md#3.6.2)  

   　　3.6.3. [PaaS-TA 설치 Shell Scripts](paas-ta_min_install_guide.md#3.6.3)  

   　　　●  [deploy-aws-4vms.sh](paas-ta_min_install_guide.md#3.6.3.1)  

   　　　●  [deploy-aws-7vms.sh](paas-ta_min_install_guide.md#3.6.3.2)  

   　3.7. [PaaS-TA 설치](paas-ta_min_install_guide.md#3.7)  

   　3.8. [PaaS-TA 설치 - 다운로드 된 Release 파일 이용 방식](paas-ta_min_install_guide.md#3.8)  

   　3.9. [PaaS-TA 로그인](paas-ta_min_install_guide.md#3.9)   
...
![](../../.gitbook/assets/bosh2-1%20%282%29%20%283%29%20%283%29.png)
...
| deploy-aws-4vms.sh | AWS 환경에 PaaS-TA-min 4vm 설치를 위한 Shell Script 파일 |
| deploy-aws-7vms.sh | AWS 환경에 PaaS-TA-min 7vm 설치를 위한 Shell Script 파일 |
| min-paasta-deployment.yml | PaaS-TA-min을 배포하는 Manifest 파일 |
...
database\_azs: \["z1"\] \# Database 가용 존 database\_instances: 1 \# Database 인스턴스 수 database\_vm\_type: "small" \# Database VM 종류 database\_network: "default" \# Database 네트워크 database\_persistent\_disk\_type: "100GB" \# Database 영구 Disk 종류

## CONTROL

control\_azs: \["z1"\] \# Control 가용 존 control\_instances: 1 \# Control 인스턴스 수 control\_vm\_type: "small-highmem-16GB" \# Control VM 종류 control\_network: "default" \# Control 네트워크 control\_vm\_extensions: \["diego-ssh-proxy-network-properties"\] \# Control VM 확장

## ROUTER

router\_azs: \["z1"\] \# Router 가용 존 router\_instances: 1 \# Router 인스턴스 수 router\_vm\_type: "minimal" \# Router VM 종류 router\_network: "default" \# Router 네트워크 router\_vm\_extensions: \["cf-router-network-properties"\] \# Router VM 확장

## COMPUTE

compute\_azs: \["z1"\] \# COMPUTE 가용 존 compute\_instances: 1 \# COMPUTE 인스턴스 수 compute\_vm\_type: "small-highmem-16GB" \# COMPUTE VM 종류 compute\_network: "default" \# COMPUTE 네트워크 compute\_vm\_extensions: \["100GB\_ephemeral\_disk"\] \# COMPUTE VM 확장
...
tcp\_router\_azs: \["z1"\] \# TCP-Router 가용 존 tcp\_router\_instances: 1 \# TCP-Router 인스턴스 수 tcp\_router\_vm\_type: "minimal" \# TCP-Router VM 종류 tcp\_router\_network: "default" \# TCP-Router 네트워크 tcp\_router\_vm\_extensions: \["cf-tcp-router-network-properties"\] \# TCP-Router VM 확장
...
| operations/use-offline-releases.yml | min-paasta-deployment.yml에서 사용되는 릴리즈를 오프라인에 저장된 릴리즈로 설치가 가능하다. | Requires value : -v releases\_dir |
| operations/min-use-postgres.yml | Database를 Postgres로 설치  - min-use-postgres.yml 미적용 시 MySQL 설치  - 3.5 이전 버전에서 Migration 시 필수 |  |
| operations/use-compiled-releases-postgres.yml | 인터넷이 연결된 환경에서 컴파일 없이 Postgres의 빠른 설치가 가능하다. | Requires: use-postgres.yml |
| operations/use-offline-releases-postgres.yml | min-use-postgres.yml에서 사용되는 릴리즈를 오프라인에 저장된 릴리즈로 설치가 가능하다. | Requires: min-use-postgres.yml  Requires value : -v releases\_dir |
| operations/min-use-haproxy.yml | HAProxy 적용  - IaaS에서 제공하는 LB를 사용하여 PaaS-TA 설치 시, Operation 파일을 제거하고 설치한다. | Requires operation file: use-haproxy-public-network.yml  Requires value : -v haproxy\_private\_ip |
| operations/use-haproxy-public-network.yml | HAProxy Public Network 설정  - IaaS에서 제공하는 LB를 사용하여 PaaS-TA 설치 시, Operation 파일을 제거하고 설치한다. | Requires: min-use-haproxy.yml  Requires Value :  -v haproxy\_public\_ip  -v haproxy\_public\_network\_name |
| operations/use-compiled-releases-haproxy.yml | 인터넷이 연결된 환경에서 컴파일 없이 HAProxy의 빠른 설치가 가능하다. | Requires: min-use-haproxy.yml |
| operations/use-offline-releases-haproxy.yml | min-use-haproxy.yml에서 사용되는 릴리즈를 오프라인에 저장된 릴리즈로 설치가 가능하다. | Requires: min-use-haproxy.yml  Requires value : -v releases\_dir |
| operations/min-use-router-public-network.yml | router를 외부 접근을 가능하게 수정한다. | 4VMs 배포시 사용 |
| operations/min-create-vm-singleton-blobstore.yml | 4VMs에서 사용되는 database의 singleton-blobstore를 단일 VM으로 배포한다. | Requires: min-use-haproxy.yml  7VMs 배포시 사용  Requires operation file: min-option-network-and-deployment.yml |
| operations/min-create-vm-tcp-router.yml | 4VMs에서 사용되는 router의 tcp-router를 단일 VM으로 배포한다. | 7VMs 배포시 사용 |
...
* 서버 환경에 맞추어 [common\_vars.yml](paas-ta_min_install_guide.md#3.6.1.1)와 \[vars.yml\]\(\(\#3.6.1.2\)\)을 수정 한 뒤, Deploy 스크립트 파일의 설정을 수정한다.
* 4VM 배포시
...
> $ bosh -e ${BOSH\_ENVIRONMENT} vms -d paasta
